...
BEGIN
        ...
        SELECT @BusinessHours = @BusinessHours - 24*count(Holiday_Date) from [dbo].[CS_HOLIDAYS] where Holiday_Date >= @DateFrom and HOLIDAY_DATE <= @DateTo
...
BEGIN
        ...
        SELECT @BusinessDays = @BusinessDays - count(Holiday_Date) from [dbo].[CS_HOLIDAYS] where Holiday_Date >= convert(varchar,@DateFrom,101) and holiday_date <= convert(varchar,@CDate,101)
        RETURN @BusinessDays
END
GO
...
-- Description:        Returns
-- =============================================
IF OBJECT_ID (N'[dbo].[fn_CodedOwnerID]', N'FN') IS NOT NULL
    DROP FUNCTION [dbo].[fn_CodedOwnerID]
GO
CREATE FUNCTION [dbo].[fn_CodedOwnerID] (
        @Name        as varchar(128) = 'ORGANIZATION',                                    -- 3 description from categroy, 4 category from description 5
        @CodedID     as varchar(128) = ''
)
RETURNS INT
AS
BEGIN
        Declare @ID as INT = -1, @Code as varchar(16) =  dbo.fn_OwnerIDCode(@Name)
        if isNumeric(@Name) > 0
           Set @ID = Cast(@Name as int)
        else
           Set @Name = Upper(RTRIM(LTRIM(@Name)))
        if @ID > -1 return @ID

        Set @CodedID = Upper(RTRIM(LTRIM(@CodedID)))
        if len(@Name) > 0 and  len(@CodedID) > 0
        BEGIN

                if @Name = 'ORGANIZATION' or @Name = 'ORG' or  @Code = 'E_ORGANIZATION'
                     select @ID = ID from Organizations where ValueCode = @CodedID or NameCode = @CodedID
                else if @Name = 'GROUP'or @Code = 'E_GROUP'
                     select @ID = ID from CodedGroups where ValueCode = @CodedID or NameCode = @CodedID
                else if @Name = 'PERSON' or @Code= 'E_PERSON'
                     select @ID = ID from Persons where @CodedID = Upper(LastName + ', ' + FirstName) or Upper(LastName + ', ' + FirstName) like @CodedID +'%'
                else if @Name = 'PROFILE' or @Code = 'E_PROFILE'
                     select @ID = ID from Profiles where ValueCode = @CodedID or NameCode = @CodedID
                else if @Name = 'REGISTEREDCODE' or @Name = 'ESYSCODE' or @Code = 'E_ESYS_CODE'
                     select @ID = ID from [dbo].[CTEPESYSRegisteredCodes] where ValueCode = @CodedID or NameCode = @CodedID or ReferenceID = @CodedID or ReferenceName like '%;'+ @CodedID + ';%'
                else if @Name = 'ENTITY' or @Code = 'E_ENTITY'
                     select @ID = ID from RegisteredEntities where ValueCode = @CodedID or NameCode = @CodedID
                else if @Name = 'AGENT' or @Code = 'E_AGENT'
                     select @ID = ID from CodedAgents where ValueCode = @CodedID or NameCode = @CodedID
                else if @Name = 'ALIAS' or @Code = 'E_ALAIS'
                     select @ID = ID from CodedGroups where ValueCode = @CodedID or NameCode = @CodedID
                else if @Name = 'DATA' or @Code = 'E_DATA'
                     select @ID = ID from CodedData where ValueCode = @CodedID or NameCode = @CodedID
                else if @Name = 'REASON' or @Code = 'E_REASON'
                     select @ID = ID from CodedReasons where ValueCode = @CodedID or NameCode = @CodedID
                else if @Name = 'DISEASE' or @Code = 'E_DISEASE'
                     select @ID = ID from CodedDiseases where ValueCode = @CodedID or NameCode = @CodedID
                else if @Name = 'MESSAGE' or @Code = 'E_MESSAGE'
                     select @ID = ID from CodedMessages where ValueCode = @CodedID or NameCode = @CodedID
                else if @Name = 'EVENT' or @Code = 'E_EVENT'
                     select @ID = ID from CodedEvents where ValueCode = @CodedID or NameCode = @CodedID
                else if @Name = 'ROLE' or @Code = 'E_ROLE'
                     select @ID = ID from CodedRoles where ValueCode = @CodedID or NameCode = @CodedID
                else if @Name = 'DISEASE' or @Code = 'E_ERROR'
                     select @ID = ID from CodedErrors where ValueCode = @CodedID or NameCode = @CodedID
                else if @Name = 'ERROR' or @Code = 'E_REASON'
                     select @ID = ID from CodedReasons where ValueCode = @CodedID or NameCode = @CodedID
                else if @Name = 'ACTIVITY' or @Code = 'E_ACTIVITY'
                     select @ID = ID from CodedActivities where ValueCode = @CodedID or NameCode = @CodedID
                else if @Name = 'REPORT' or @Code = 'E_REPORT'
                     select @ID = ID from CodedReports where ValueCode = @CodedID or NameCode = @CodedID
                else if @Name = 'TEMPLATE'  or @Code = 'E_TEMPLATE'
                     select @ID = ID from CodedTemplates where ValueCode = @CodedID or NameCode = @CodedID
                else if @Name = 'COUNTRY' or @Code = 'E_COUNTRY'
                     select @ID = ID from CodedCountries where ValueCode = @CodedID or NameCode = @CodedID
                else if @Name = 'COUNTY' or  @Code = 'E_COUNTY'
                     select @ID = ID from CodedCounties where ValueCode = @CodedID or NameCode = @CodedID
                else if @Name = 'STATEPROVINCE' or  @Code = 'E_STAT_PROV'
                     select @ID = ID from CodedStatesProvinces where ValueCode = @CodedID or NameCode = @CodedID
                else if @Name = 'VALUESET' or @Code = 'E_VALUESET'
                     select @ID = ID from CodedValueSets where ValueCode = @CodedID or NameCode = @CodedID
                else if @Name = 'BIOMARKER' or @Code = 'E_BIOMARKER'
                     select @ID = ID from CodedBiomarkers where ValueCode = @CodedID or NameCode = @CodedID
                else if @Name = 'TYPE' or @Code = 'E_TYP_CODE'
                     select @ID = ID from CodedTypeCodes where ValueCode = @CodedID or NameCode = @CodedID
                else if @Name = 'CATEGORY' or @Code = 'E_CAT_CODE'
                     select @ID = ID from CodedCategoryCodes where ValueCode = @CodedID or NameCode = @CodedID
                else if @Name = 'LABORATORY' or @Code = 'E_LABORATORY'
                     select @ID = ID from CodedLaboratories where ValueCode = @CodedID or NameCode = @CodedID
                else if @Name = 'DATA' or @Code = 'E_DATA'
                     select @ID = ID from CodedData where ValueCode = @CodedID or NameCode = @CodedID
                else if @Name = 'POSTCODE' or @Code = 'E_POSTCODE'
                     select @ID = ID from CodedPostCodes where ValueCode = @CodedID or NameCode = @CodedID
                else if @Name = 'HOLIDAY' or @Code = 'E_HOLIDAY'
                     select @ID = ID from CodedHolidays where ValueCode = @CodedID or NameCode = @CodedID
                else if @Name = 'OUTCOMEMEASURE' or @Code = 'E_OUTCOME'
                     select @ID = ID from CodedOutcomeMeasures where ValueCode = @CodedID or NameCode = @CodedID
                else if @Name = 'REGISTRATION' or @Code = 'E_REG_CENTER'
                     select @ID = ID from CodedRegistrationCenters where ValueCode = @CodedID or NameCode = @CodedID
                else if @Name = 'MILESTONE' or @Code = 'E_MILESTONE'
                     select @ID = ID from CodedMilestones where ValueCode = @CodedID or NameCode = @CodedID
                else if @Name = 'SCHEDULER'  or @Code = 'E_SCHEDULER'
                     select @ID = ID from CodedSchedulers where ValueCode = @CodedID
                else if @Name = 'TASK'  or @Code = 'E_TASK'
                     select @ID = ID from CodedTasks where ValueCode = @CodedID
                else if @Name = 'WORKFLOW'  or @Code = 'E_WORKFLOW'
                     select @ID = ID from CodedWorkflows where ValueCode = @CodedID
                else if @Name = 'NOTIFICATION' or @Code = 'E_NOTN_CHANNEL'
                     select @ID = ID from CodedNotificationChannels where ValueCode = @CodedID or NameCode = @CodedID
                else if @Name = 'COMMENT' or @Code = 'E_COMMENT'
                     select @ID = ID from OwnedComments where NameCode + ':' +  ReferenceID = @CodedID
                else if @Name = 'CONTACT' or @Code = 'E_CONTACT'
                     select @ID = ID from OwnedContacts where NameCode + ':' +  ReferenceID = @CodedID
                else if @Name = 'SUBMISSION' or @Code = 'E_SUBMISSION'
                     select @ID = ID from SubmissionReviews where NameCode + ':' + ReferenceID = @CodedID
                else if @Name = 'SUBMISSIONUNIT' or @Code = 'E_SUBM_UNIT' or @Name = 'SUBMISSIONITEM'
                     select @ID = ID from SubmissionItems where NameCode + ':' + ReferenceID = @CodedID
                else if @Name = 'QUEUENOTIFICATION' or @Code = 'E_NOTN_QUEUE'
                     select @ID = ID from QueuedNotifications where NameCode = @CodedID or URL = @CodedID
                else if @Name = 'OWNEDTASK' or @Code = 'E_OWN_TASK'
                     select @ID = ID from OwnedTasks where NameCode = @CodedID or ReferenceID = @CodedID
                else if @Name = 'OWNEDMESSAGE' or @Code = 'E_OWN_MESSAGE'
                     select @ID = ID from OwnedMessages where NameCode = @CodedID or ReferenceID = @CodedID
                else if @Name = 'OWNEDTWORKFLOW' or @Code = 'E_OWN_WORKFLOW'
                     select @ID = ID from OwnedWorkflows where NameCode = @CodedID or ReferenceID = @CodedID
                else if @Name = 'OWNEDREPORT' or @Code = 'E_OWN_REPORT'
                     select @ID = ID from OwnedReports where NameCode = @CodedID or ReferenceID = @CodedID
                else if @Name = 'OWNEDEVENT' or @Code = 'E_OWN_EVENT'
                     select @ID = ID from OwnedEvents where NameCode = @CodedID or ReferenceID = @CodedID
                else if @Name = 'OWNEDERROR' or @Code = 'E_OWN_ERROR'
                     select @ID = ID from OwnedErrors where NameCode = @CodedID or ReferenceID = @CodedID
                else
                     set @ID = 0
        END
        RETURN @ID
...
GO
GRANT EXECUTE ON [dbo].[fn_CodedOwnerID]
    TO CTEPESYS_UserRole;
GO
...
-- Description:        Returns
-- =============================================
IF OBJECT_ID (N'[dbo].[fn_ProfileID]', N'FN') IS NOT NULL
    DROP FUNCTION [dbo].[fn_ProfileID]
GO
CREATE FUNCTION [dbo].[fn_ProfileID] (
        @O  as varchar(16) = 'E_ORGANIZATION',
        @ID as Int = 0,
        @RI as varchar(16) =''
)
RETURNS int
AS
BEGIN
        Declare @RID as int = 0
        if @ID > 0
        BEGIN
                if @O = 'E_ORGANIZATION'
                   select @RID = ProfileID from Organizations where ID = @ID and isnull(ReferenceID,'') = @RI
                else if @O = 'E_ROLE'
                   select @RID = ProfileID from CodedRoles where ID = @ID
                else if @O = 'E_GROUP'
                   select @RID = ProfileID from CodedGroups where ID = @ID
                else if @O = 'E_PERSON'
                   select @RID = ProfileID from Persons where ID = @ID
                else if @O = 'E_PROFILE'
                   select @RID = ID From Profiles where OwnerID = @ID and OwnerIDCode = @RI
               /*
                else if @O = 'E_STUDY'
                   select @RID = ProfileID from StudyProtocols where ID = @ID
                else if @O = 'E_STY_VERSION'
                   select @RID = s.ProfileID from StudyProtocols s inner join StudyProtocolVersions v on s.ID = v.StudyID where v.ID = @ID
               */
        END
        RETURN @RID
END
GO
GRANT EXECUTE ON [dbo].[fn_ProfileID]
    TO CTEPESYS_UserRole;
GO
...
-- Author:
-- Create date:
-- Description:        Returns   referehced based ID for various objects
-- =============================================
IF OBJECT_ID (N'[dbo].[fn_ReferenceCodedOwnerID]', N'FN') IS NOT NULL
    DROP FUNCTION [dbo].[fn_ReferenceCodedOwnerID]
GO
CREATE FUNCTION [dbo].[fn_ReferenceCodedOwnerID] (
        @ReferenceID as varchar(64) = '',
        @Name        as varchar(128) = '',                                    -- 3 description from categroy, 4 category from description 5
        @CodedID     as varchar(128) = ''
)
RETURNS INT
AS
BEGIN
        ...
        Set @Name = Upper(RTRIM(LTRIM(@Name)))
        Set @CodedID = Upper(RTRIM(LTRIM(@CodedID)))
        Set @ReferenceID = Upper(RTRIM(LTRIM(@ReferenceID)))
        if len(@Name) > 0 and  len(@CodedID) > 0
        BEGIN
                if @Name = 'ORGANIZATION' or @Name = 'ORG' or dbo.fn_OwnerIDCode(@Name) = 'E_ORGANIZATION'
                BEGIN
                     if len(@ReferenceID) > 0
                       select @ID = ID from Organizations where (ValueCode = @CodedID or NameCode = @CodedID ) and ReferenceID = @ReferenceID
                     else
                       select @ID = ID from Organizations where ValueCode = @CodedID or NameCode = @CodedID
                END
                else if @Name = 'GROUP' or @Name = 'CODEDGROUPS' or dbo.fn_OwnerIDCode(@Name) = 'E_GROUP'
                BEGIN
                     if len(@ReferenceID) > 0
                             select @ID = ID from CodedGroups where (ValueCode = @CodedID or NameCode = @CodedID  ) and ReferenceID = @ReferenceID
                     else
                             select @ID = ID from CodedGroups where ValueCode = @CodedID or NameCode = @CodedID
                END
                else if @Name = 'PROFILE' or @Name = 'PROFILES' or dbo.fn_OwnerIDCode(@Name) = 'E_PROFILE'
                BEGIN
                     if len(@ReferenceID) > 0
                            select @ID = ID from Profiles where (ValueCode = @CodedID or NameCode = @CodedID ) and ReferenceID = @ReferenceID
                      else
                            select @ID = ID from Profiles where ValueCode = @CodedID or NameCode = @CodedID
                END
                else if @Name = 'AGENT' or @Name = 'CODEDAGENTS' or dbo.fn_OwnerIDCode(@Name) = 'E_AGENT'
                BEGIN
                     if len(@ReferenceID) > 0
                           select @ID = ID from CodedAgents where (ValueCode = @CodedID or NameCode = @CodedID ) and ReferenceID = @ReferenceID
                     else
                           select @ID = ID from CodedAgents where ValueCode = @CodedID or NameCode = @CodedID
                END
                else if @Name = 'ALIAS' or @Name = 'CODEDALIASES'  or dbo.fn_OwnerIDCode(@Name) = 'E_ALAIS'
                BEGIN
                     if len(@ReferenceID) > 0
                        select @ID = ID from CodedGroups where (ValueCode = @CodedID or NameCode = @CodedID ) and ReferenceID = @ReferenceID
                     else
                        select @ID = ID from CodedGroups where ValueCode = @CodedID or NameCode = @CodedID
                END
                else if @Name = 'DATA' or @Name = 'CODEDDATA'  or dbo.fn_OwnerIDCode(@Name) = 'E_DATA'
                BEGIN
                     if len(@ReferenceID) > 0
                         select @ID = ID from CodedData where (ValueCode = @CodedID or NameCode = @CodedID ) and ReferenceID = @ReferenceID
                     else
                         select @ID = ID from CodedData where ValueCode = @CodedID or NameCode = @CodedID
                END
                else if @Name = 'REASON' or @Name = 'CODEDREASONS'  or dbo.fn_OwnerIDCode(@Name) = 'E_REASON'
                BEGIN
                      if len(@ReferenceID) > 0
                          select @ID = ID from CodedReasons where (ValueCode = @CodedID or NameCode = @CodedID ) and ReferenceID = @ReferenceID
                      else
                          select @ID = ID from CodedReasons where ValueCode = @CodedID or NameCode = @CodedID
                END
                else if @Name = 'DISEASE' or @Name = 'CODEDDISEASES'  or dbo.fn_OwnerIDCode(@Name) = 'E_DISEASE'
                BEGIN
                      if len(@ReferenceID) > 0
                          select @ID = ID from CodedDiseases where (ValueCode = @CodedID or NameCode = @CodedID ) and ReferenceID = @ReferenceID
                      else
                          select @ID = ID from CodedDiseases where ValueCode = @CodedID or NameCode = @CodedID
                END
                else if @Name = 'MESSAGE' or @Name = 'CODEDMESSAGES'  or dbo.fn_OwnerIDCode(@Name) = 'E_MESSAGE'
                BEGIN
                      if len(@ReferenceID) > 0
                          select @ID = ID from CodedMessages where (ValueCode = @CodedID or NameCode = @CodedID ) and ReferenceID = @ReferenceID
                      else
                          select @ID = ID from CodedMessages where ValueCode = @CodedID or NameCode = @CodedID
                END
                else if @Name = 'EVENT' or @Name = 'CODEDEVENTS'  or dbo.fn_OwnerIDCode(@Name) = 'E_EVENT'
                BEGIN
                     if len(@ReferenceID) > 0
                          select @ID = ID from CodedEvents where (ValueCode = @CodedID or NameCode = @CodedID ) and ReferenceID = @ReferenceID
                     else
                          select @ID = ID from CodedEvents where ValueCode = @CodedID or NameCode = @CodedID
                END
                else
...
GO
GRANT EXECUTE ON [dbo].[fn_ReferenceCodedOwnerID]
    TO CTEPESYS_UserRole;
GO
...
-- Description:        Returns
-- =============================================
IF OBJECT_ID (N'[dbo].[fn_WhatIsRegisteredCode]', N'FN') IS NOT NULL
    DROP FUNCTION [dbo].[fn_WhatIsRegisteredCode]
GO
CREATE FUNCTION [dbo].[fn_WhatIsRegisteredCode] (
...
BEGIN
        ...
        BEGIN
                if @Switch =  0
                BEGIN
                        select @Data = [ID] from [dbo].[CTEPESYSRegisteredCodes] where Upper(NameCode) = @TableName
                END
                else if @Switch =  1
                BEGIN
                        select @Data = [ClassCode] from [dbo].[CTEPESYSRegisteredCodes] where Upper(NameCode) = @TableName
                END
                else if @Switch =  2
                BEGIN
                        select @Data = [CategoryCode] from [dbo].[CTEPESYSRegisteredCodes] where Upper(NameCode) = @TableName
                END
                else if @Switch =  3
                BEGIN
                        select @Data = [TypeCode] from [dbo].[CTEPESYSRegisteredCodes] where Upper(NameCode) = @TableName
                END
                else if @Switch =  4
                BEGIN
                        select @Data = [Alias] from [dbo].[CTEPESYSRegisteredCodes] where Upper(NameCode) = @TableName
                END
                else if @Switch =  9
                BEGIN
                        select @Data = [Description] from [dbo].[CTEPESYSRegisteredCodes] where Upper(NameCode) = @TableName
                END
                else if @Switch =  10
                BEGIN
                        select @Data = [NameCode] from [dbo].[CTEPESYSRegisteredCodes] where Upper(NameCode) = @TableName
                END
                else
                BEGIN
                        select @Data = [ValueCode] from [dbo].[CTEPESYSRegisteredCodes] where Upper(NameCode) = @TableName
                END
        END
        RETURN @Data
ErrorHandle:
...
GO
GRANT EXECUTE ON [dbo].[fn_WhatIsRegisteredCode]
    TO CTEPESYS_UserRole;
GO
...
-- Description:        Returns
-- =============================================
IF OBJECT_ID (N'[dbo].[fn_CFGItem]', N'FN') IS NOT NULL
    DROP FUNCTION [dbo].[fn_CFGItem]
GO
CREATE FUNCTION [dbo].[fn_CFGItem] (
        @CategoryCode  as varchar(128) = NULL,
        @TypeCode as varchar(16) = NULL,
        @ProfileCode as varchar(16) = 'C_CTEPEYS',
        @ReturnCode as varchar(16) = 'C_VALUE'
)
RETURNS Varchar(256)
AS
BEGIN
        Declare @RN as varchar(256) = '', @PID as int = NULL

        if isnumeric(@ProfileCode) > 0
           set @PID = cast(@ProfileCode as int)
        else
           set @PID = dbo.fn_CodedOwnerID('Profile',@ProfileCode)

        if @ReturnCode = 'C_CODE'
               Select @RN = ValueCode
               from ConfigurationItems where ProfileID = isNull(@PID,ProfileID) and (CategoryCode = @CategoryCode and TypeCode = @TypeCode)
        else if @ReturnCode = 'C_INTEGER'
               Select @RN = cast(IntegerValue as varchar(16))
               from ConfigurationItems where ProfileID = isNull(@PID,ProfileID) and (CategoryCode = @CategoryCode and TypeCode = @TypeCode)
        else if @ReturnCode = 'C_VALUE' or  @ReturnCode = 'C_STRING'
               Select @RN = StringValue
               from ConfigurationItems where ProfileID = isNull(@PID,ProfileID) and (CategoryCode = @CategoryCode and TypeCode = @TypeCode)
        else if @ReturnCode = 'C_CHECK' or @ReturnCode = 'C_CHECKBOX'
               Select @RN = cast(CheckValue as varchar(16))
               from ConfigurationItems where ProfileID = isNull(@PID,ProfileID) and (CategoryCode = @CategoryCode and TypeCode = @TypeCode)
        else if @ReturnCode = 'C_NM_CODE' or  @ReturnCode = 'C_NAME'
               Select @RN = NameCode
               from ConfigurationItems where ProfileID = isNull(@PID,ProfileID) and (CategoryCode = @CategoryCode and TypeCode = @TypeCode)
        else if @ReturnCode = 'C_DESCRIPTION'
               Select @RN = Description
               from ConfigurationItems where ProfileID = isNull(@PID,ProfileID) and (CategoryCode = @CategoryCode and TypeCode = @TypeCode)
        else if @ReturnCode = 'C_COMMENT'
               Select @RN = Comments
               from ConfigurationItems where ProfileID = isNull(@PID,ProfileID) and (CategoryCode = @CategoryCode and TypeCode = @TypeCode)
        else if @ReturnCode = 'C_ID_REFERENCE'
               Select @RN = ReferenceID
               from ConfigurationItems where ProfileID = isNull(@PID,ProfileID) and (CategoryCode = @CategoryCode and TypeCode = @TypeCode)
        else if @ReturnCode = 'C_NM_REFERENCE'
               Select @RN = ReferenceName
               from ConfigurationItems where ProfileID = isNull(@PID,ProfileID) and (CategoryCode = @CategoryCode and TypeCode = @TypeCode)
        else if @ReturnCode = 'C_ACCESSLEVEL'
               Select @RN = cast(AccessLevel as varchar(16))
               from ConfigurationItems where ProfileID = isNull(@PID,ProfileID) and (CategoryCode = @CategoryCode and TypeCode = @TypeCode)
        else
               Select @RN = StringValue
               from ConfigurationItems where ProfileID = isNull(@PID,ProfileID) and (CategoryCode = @CategoryCode and TypeCode = @TypeCode)
...
END
GO
GRANT EXECUTE ON [dbo].[fn_CFGItem]
    TO CTEPESYS_UserRole;
GO
...
SET QUOTED_IDENTIFIER ON
GO
IF OBJECT_ID('[dbo].[fn_IsAudiitReady]', 'FN') IS NOT NULL
   DROP FUNCTION [dbo].[fn_IsAudiitReady]
GO
/*============================================================*/
/* Created By Peter Yan on Date:                              */
/* Description:                                               */
/*============================================================*/
CREATE FUNCTION [dbo].[fn_IsAudiitReady]
(
...
BEGIN
  ...
  BEGIN CATCH
        ...
        BEGIN
                ...
                exec sp_DBLog_Message @Method='[dbo].[fn_IsAudiitReady]', @Class=@TRP_ID, @Description=@ErrMsg, @ErrorMessage=@errMsg, @LoggedInUserID=@SenderID, @Mail='No'
        END
        RETURN @V_RV
  END CATCH
END;
GO
GRANT EXECUTE ON [dbo].[fn_IsAudiitReady]
    TO eGOS_UserRole
GO
...
CREATE PROCEDURE [dbo].[sp_GetIDNumber]
  ...
  @P_TYPE      as varchar(16)   = 'C_ORDER',             -- PROPOSAL
  @P_APP       as varchar(64)   = 'C_SP',                -- C_CS, C_GSS, C_SP3,   ==> SPX_, SP3_, GSS_, CSX_
...
BEGIN
  ...
  /* enter code here... */
  if length(IsNULL(@P_APP,'C_SP')) = 64   set @P_APP = 'C_' + replace(LEFT(@APP,3),'X','')
  ...
  BEGIN
          SELECT @ReturnValue = ACRONYM_NAME + '_' +  RIGHT('00000000'+Cast(PROPOSAL_NUMBER as varchar(8)),8) From CONTRACTS WHERE ORGANIZATION_IDF = @P_ORG_IDF AND ISNULL(LOCKED_BY,'') = '' AND STATUS_CODE = 'ACTIVE'
          if @@ROWCOUNT > 0
          BEGIN
                UPDATE CONTRACTS Set LAST_PROPOSAL_NUMBER = PROPOSAL_NUMBER, PROPOSAL_NUMBER = PROPOSAL_NUMBER + 1, LOCKED_BY = @P_USER, LOCKED_DATE = @P_DATE, LOCKED_EXPIRED_DATE = DateAdd(Day, 1, @P_DATE), UPDATED_BY = @P_USER, UPDATED_DATE = @P_DATE, UPDATED_COUNT = UPDATED_COUNT + 1
                WHERE ORGANIZATION_IDF = @P_ORG_IDF AND ISNULL(LOCKED_BY,'') = '' AND STATUS_CODE = 'ACTIVE'
          END
  END
 --- ELSE IF @P_TYPE IN ('','')
END;
