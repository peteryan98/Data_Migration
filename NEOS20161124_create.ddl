...
BEGIN
     ...
     BEGIN
             ...
             if iSNumeric(right(@P_Code,1)) > 0
                 select @v_RV &= MASK_VALUE from dbo.CONTRACT_AREAS where VALUE_CODE = @P_Code
             else if len(@p_Code) < 6
                 select @v_RV &= MASK_VALUE from dbo.CODE_CATEGORY_TYPES where VALUE_CODE = @P_Code
             else
                 select @v_RV &= MASK_VALUE from dbo.CONTRACT_AREAS  where VALUE_CODE = @P_Code
