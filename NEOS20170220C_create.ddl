...
BEGIN
        ...
        if Len(isNull(@P_ID,'')) > ''
               SET @P_ID = @tz + right('0000'+@P_ID,5)
        ELSE
               SET @P_ID = @tz + Right(@vID,5)

        set @v_RV = Left(@v_RV,26)+ @P_ID + RIGHT(@v_RV,32)
        RETURN @v_RV
END;
GO
...
AS
BEGIN
        DECLARE @V_RV as varchar(64) = LTRIM(RTRIM(@P_IDF))
        if len(@V_RV) > 32 SET @v_RV = SUBSTRING(@v_RV,28,5)
        RETURN @V_RV
END;
GO
...
BEGIN
        ...
        BEGIN
                ...
                BEGIN
                        ...
                        WHEN 'C_FIRST'   THEN                SUBSTRING(@Data,1, @Pos)
                        WHEN 'C_RIGHT'   THEN                SUBSTRING(@Data, @Pos+1, 1024)
...
IF OBJECT_ID('[dbo].[fn_GetPerson]', 'FN') IS NOT NULL
...
GO
GRANT EXECUTE ON [dbo].[fn_GetPerson]
    TO NEOS_UserRole
GO

--   [dbo].[fn_GetPerson](@IDF=, @Code)
...
BEGIN
  ...
  BEGIN
      ...
      END
  END
  ELSE if @P_TYPE IN ('C_REVISION','C_MOD')
  BEGIN
      Set @P_TYPE = 'C_MOD'
      Set @Cat = 'C_ALL'
      Set @Prefix = 'MOD'
      set @Name = 'Contract Modification (MOD) ID: '
      SELECT @RefCode = VALUE_CODE FROM REF_NITAAC_IDS WHERE STATUS_CODE = 'COMPLETE' AND VALUE_CODE = @P_REF_ID AND TYPE_CODE in ('C_AWARD','C_MOD')
      if @@ROWCOUNT = 0
      BEGIN
  ...
  BEGIN
          ...
          RETURN @ReturnValue
  END
  ELSE IF @P_TYPE in ('C_AMEND', 'C_REVISION')
  BEGIN
      Set @P_TYPE = 'C_AMEND'
      Set @Prefix = 'AMND'
      ...
      SELECT @IDNo = count(*) FROM REF_NITAAC_IDS WHERE VALUE_CODE = @RefCode and STATUS_CODE in ('COMPLETE','ACTIVE') AND TYPE_CODE in ('C_ORDER','C_MOD')
      if @IDNo > 0
      BEGIN
              SELECT @IDNo = count(*) + 1 FROM REF_NITAAC_IDS WHERE PARENT_CODE = @P_REF_ID and STATUS_CODE = 'COMPLETE'
              SET @ReturnValue = @P_REF_ID + '_'+Cast(@IDNo as varchar(2))
              ...
              FROM REF_NITAAC_IDS WHERE STATUS_CODE in ('COMPLETE','ACTIVE') AND VALUE_CODE = @RefCode AND TYPE_CODE in ('C_ORDER','C_MOD')
      END
      ELSE
  ...
  BEGIN
       ...
       BEGIN
            BEGIN TRANSACTION
                     ...
                     Set @ReturnValue = @Prefix+RIGHT('0000000'+cast(@IDno as varchar(7)),7)+@Suffix
                     ...
                     VALUES(dbo.fn_NewIDFExt(@Prefix,default, cast(1000001-@IDNo as varchar(5))),@IDNo, @IDNo+1, @P_STATUS, @ReturnValue, IsNUll(@P_ORG_IDF,'')+ ': ID Gnerated: '+@P_APP, IsNUll(@P_ORG_IDF,'') + ' Customer requested ID Generated for: ' + @P_APP, @Cat, @P_TYPE, @P_APP, @Prefix, @Suffix, @P_DATE,@P_USER, @IDF, @P_ORG_IDF, @ReturnValue, IsNull(@RefCode, @P_REF_ID), @P_REF_NAME, @P_ACCESS)
